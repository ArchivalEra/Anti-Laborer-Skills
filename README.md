...
| **`anti-laborer-output`** | 话语生成与解构烹饪技能：前置 Matt 风格 Step 0 盘问探底（Grill-Me）压测决策树前沿；将底层知识库（OKF v0.2）作为只读食材库，传授并执行即时烹饪手法；按场景看客下菜、把控火候、双遍去腥自检，生成无反调情绪、高信息密度的理性回应。 | [SKILL.md](./skills/anti-laborer-output/SKILL.md) | [SKILL.en.md](./skills/anti-laborer-output/SKILL.en.md) | ✅ 已就绪 |
